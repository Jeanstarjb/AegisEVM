...
from aegisevm.exceptions import CriticalError
from aegisevm.support.lock import LockFile
...
class MythrilConfig:
    ...
    def _add_dynamic_loading_option(config: ConfigParser) -> None:
        """
        Sets the dynamic loading config option in .aegisevm/config.ini file
        :param config: The config file object
        :return: None
        """
        config.set(
            ...
            if network in layer_one + layer_two:
                if self.infura_id in (None, ""):
                    log.info(
                        ...
                        "or in the ~/.aegisevm/config.ini file"
                    )
                    self.eth = None
                    return
