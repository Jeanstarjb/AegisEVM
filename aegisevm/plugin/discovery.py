...
from aegisevm.plugin.interface import MythrilPlugin
...
class PluginDiscovery(object, metaclass=Singleton):
    ...
    # Installed plugins structure. Retrieves all modules that have an entry point for aegisevm.plugins
    ...
    def init_installed_plugins(self):
        if pkg_resources:
            self._installed_plugins = {
                ...
                for entry_point in pkg_resources.iter_entry_points("aegisevm.plugins")
            }
        else:
            all_entry_points = entry_points()
            mythril_plugins = [
                ep for ep in all_entry_points if ep.group == "aegisevm.plugins"
            ]
